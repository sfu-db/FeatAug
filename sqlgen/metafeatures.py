...
class MissingValues(HelperFunction):
    ...
    def _calculate_sparse(self, X, logger):
...
class PercentageOfInstancesWithMissingValues(MetaFeature):
    def _calculate(self, X, logger):
        ...
        n_total = float(metafeatures["NumberOfInstances"](X, logger).value)
...
class PercentageOfFeaturesWithMissingValues(MetaFeature):
    def _calculate(self, X, logger):
        ...
        n_total = float(metafeatures["NumberOfFeatures"](X, logger).value)
        return float(n_missing / n_total)
...
#    def _calculate(self, X, y, logger):
#        return len(X[0])
...
class DatasetRatio(MetaFeature):
    def _calculate(self, X, logger):
        return float(metafeatures["NumberOfFeatures"](X, logger).value) /\
            float(metafeatures["NumberOfInstances"](X, logger).value)
...
class InverseDatasetRatio(MetaFeature):
    def _calculate(self, X, logger):
        return float(metafeatures["NumberOfInstances"](X, logger).value) /\
            float(metafeatures["NumberOfFeatures"](X, logger).value)
...
def calculate_all_metafeatures(X, dataset_name, logger,
    ...
    while len(to_visit) > 0:
        ...
        value = metafeatures[name](X_, logger)
