...
class SQLGen(object):
    ...
    def optimize(
        ...
        for temp_seed in seed_list:
            trials = Trials()
            best = fmin(fn=self._spearman_objective_func, space=fspace, algo=tpe.suggest, max_evals=500, trials=trials,
        ...
        for i in range(len(self.query_template.all_predicate_attrs)):
            ...
            for temp_seed in seed_list:
                trials = Trials()
                best = fmin(fn=self._spearman_objective_func, space=fspace, algo=tpe.suggest, max_evals=500,
        ...
        for query_template in topk_query_templates:
            ...
            if turn_on_mi and mi_topk > 0:
                trials = Trials()
                best = fmin(fn=self._spearman_objective_func, space=fspace, algo=tpe.suggest, max_evals=mi_budget,
