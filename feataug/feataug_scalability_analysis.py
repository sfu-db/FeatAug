...
def init_worker():
    # Limit each worker to 16GB of memory
    maxsize = 40 * 1024 * 1024 * 1024  # 16 GB in bytes
    resource.setrlimit(resource.RLIMIT_AS, (maxsize, resource.RLIM_INFINITY))
...
class SQLGen(object):
    ...
    def evaluate_attribute(self, index, all_predicate_attrs, all_predicate_attrs_type, seed_list):
        # print("Enter parallel")
        temp_predicate_attrs = [all_predicate_attrs[index]]
        print(f"Enter parallel: {str(temp_predicate_attrs)}")
        temp_predicate_attrs_type = {temp_predicate_attrs[0]: all_predicate_attrs_type[temp_predicate_attrs[0]]}

        try:
            fspace = self._define_search_space(temp_predicate_attrs, temp_predicate_attrs_type)
            temp_mi_score = 0
            ...
            temp_mi_score /= len(seed_list)
            print(f"Complete parallel! {index}_{str(temp_predicate_attrs)}")
            return {"predicate_attrs": temp_predicate_attrs, "predicate_attrs_type": temp_predicate_attrs_type, "mi_score": temp_mi_score}
        except:
            print(f"Complete parallel! {index}_{str(temp_predicate_attrs)}")
            return {"predicate_attrs": temp_predicate_attrs, "predicate_attrs_type": temp_predicate_attrs_type, "mi_score": 0.0}
    
    def evaluate_attribute_in_layer(self, eva, seed_list):
        # print("Enter parallel in layer")
        print(eva["predicate_attrs"])
        temp_predicate_attrs = eva["predicate_attrs"]
        print(f"Enter parallel in layer: {str(temp_predicate_attrs)}")
        temp_predicate_attrs_type = eva["predicate_attrs_type"]
        print(list(temp_predicate_attrs_type))
        ...
        try:
            ...
            for temp_seed in seed_list:
                ...
                temp_mi_score += temp_best_mi_value
            temp_mi_score /= len(seed_list)
            print(f"Complete parallel in layer! {str(temp_predicate_attrs)}")
    ...
    def optimize(
        ...
        qti_start = time.time()
        fspace = self._define_search_space(
            predicate_attrs = self.query_template.predicate_attrs,
            predicate_attrs_type = self.query_template.predicate_attrs_type
        )
        temp_mi_score_empty_set = 0
        one_qti_start = time.time()
        for temp_seed in seed_list:
            trials = Trials()
        ...
        one_qti_end = time.time()
        print(f"ONE QTI TIME: {one_qti_end - one_qti_start}")
        ...
        attr_nums = len(self.query_template.all_predicate_attrs)
        print(f"ATTR NUMS: {attr_nums}")

        with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = [executor.submit(self.evaluate_attribute, i, \
                        self.query_template.all_predicate_attrs, \
                        self.query_template.all_predicate_attrs_type, seed_list)
                    for i in range(attr_nums)]
            for future in futures:
                try:
                    result = future.result()
                    print(result["predicate_attrs"], result["mi_score"])
                    all_evaluations_current_layer.append(result)
                except: 
                    print('Unable to get the result')

        all_evaluations_current_layer = sorted(all_evaluations_current_layer, key=lambda x: x["mi_score"], reverse=False)
        ...
        for j in range(1, layer_num):
            ...
            with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
                futures = [executor.submit(self.evaluate_attribute_in_layer, eva, seed_list)
                        for eva in selected_attrs]
                for future in futures:
                    try:
                        result = future.result()
                        print(result["predicate_attrs"], result["mi_score"])
                        all_real_evaluations_current_layer.append(result)
                    except:
                        print('Unable to get the result')
        ...
        qti_end = time.time()
        self.qti_time += qti_end - qti_start
        print(f"Query Template Identification Time: {self.qti_time}")
        ...
        for query_template in topk_query_templates:
            ...
            self.warmup_time += warmup_end - warmup_start
            ...
            while outer_budget_single_query_template > 0:
                ...
                best = fmin(
                    ...
                    space=fspace, 
                    algo=partial(tpe.suggest, n_startup_jobs=mi_topk), 
                    max_evals=(base_tpe_budget + mi_topk),
                ...
                # self.base_tabouter_budget_singlele = self.base_table.merge(
    ...
    def _mi_objective_func(self, params):
        ...
        # del df_with_fkeys_and_join_keys 
        # # 执行垃圾收集器，帮助释放内存
        # import gc
        # gc.collect()

        # del new_feature_after_join
        # # 执行垃圾收集器，帮助释放内存
        # import gc
        # gc.collect()
    ...
    def _define_search_space(
        self,
        predicate_attrs: Optional[List] = [],
        predicate_attrs_type: Any = {}
    ) -> Optional[Dict]:
        search_space = {}
        ...
        for predicate_attr in predicate_attrs:
            predicate_type = predicate_attrs_type[predicate_attr][
                "type"
            ]
            if predicate_type == "categorical":
                predicate_choices = predicate_attrs_type[
                    predicate_attr
                ]["choices"]
                ...
                predicate_low = predicate_attrs_type[
                    predicate_attr
                ]["low"]
                predicate_high = predicate_attrs_type[
                    predicate_attr
                ]["high"]
                ...
                predicate_q = predicate_attrs_type[predicate_attr][
                    "q"
                ]
    ...
    def _generate_new_feature(self, arg_dict: Dict = {}):
        ...
        # Extract PREDICATE ATTRS from arg dictionary
        predicate_attrs = []
        for item in list(arg_dict.keys()):
            if item.startswith('agg_func'):
                continue
            if item.startswith('agg_attr'):
                continue
            if item.startswith('groupby_keys'):
                continue
            if item.startswith('predicate_attr'):
                temp_attr = item.replace('predicate_attr_', '').replace('_bound1', '').replace('_bound2','')
                if temp_attr not in predicate_attrs:
                    predicate_attrs.append(temp_attr)
        where_clause_in_sql = ""
        predicate_attrs_label_pos = 0
        for i in range(len(predicate_attrs)):
            predicate_attr = predicate_attrs[i]
            predicate_type = self.query_template.all_predicate_attrs_type[predicate_attr][
                "type"
            ]
            if predicate_type == "categorical":
