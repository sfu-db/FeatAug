...
class SQLGen(object):
    ...
    def optimize(
        ...
        # Without query template identification

        for i in range(query_template_num):

            outer_budget_single_query_template = outer_budget

            temp_predicate_attr_len = random.randint(1, len(self.query_template.all_predicate_attrs))

            self.query_template.predicate_attrs = random.sample(self.query_template.all_predicate_attrs, temp_predicate_attr_len)
            self.query_template.predicate_attrs_type = {}
            for temp_attr in self.query_template.predicate_attrs:
                self.query_template.predicate_attrs_type[temp_attr] = self.query_template.all_predicate_attrs_type[temp_attr]
            fspace = self._define_search_space()

            if turn_on_mi and mi_topk > 0:
                trials = Trials()
                best = fmin(fn=self._mi_objective_func, space=fspace, algo=rand.suggest, max_evals=mi_budget,
            ...
            while outer_budget_single_query_template > 0:
                ...
                best = fmin(fn=self._objective_func, space=fspace, algo=rand.suggest, max_evals=(base_tpe_budget + mi_topk),
