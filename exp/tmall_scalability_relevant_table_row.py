...
# from sqlgen.select_proxy_randomly_sqlgen import QueryTemplate, SQLGen
...
def load_data(SAMPLE_ROW_NUM_RELEVANT_TABLE: int = 1300000):
    path = "../exp_data/Tmall/"
    ...
    user_log_sample = user_log_sample.sample(n=SAMPLE_ROW_NUM_RELEVANT_TABLE)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-rr', '--sampled_row_num', type=int, default=1300000, help = "ML model",required = False)
    args = parser.parse_args()
    ml_model = args.ml_model
    sampled_row_num = args.sampled_row_num

    train_data, train_labels, valid_data, valid_labels, test_data, test_labels, user_log = load_data(SAMPLE_ROW_NUM_RELEVANT_TABLE=sampled_row_num)
    ...
    # for x in predicate_attrs:
    #     print(x)
    #     print(len([str(x) for x in user_log[x].unique()]))
