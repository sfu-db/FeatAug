...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--col_num', type=int, default=20, help = "ML model",required = False)
        ...
        while len(predicate_attrs) < SAMPLE_COL_NUM:
            ...
            if not col in predicate_attrs:
                if not (col.startswith('session0') or \
                        ...
                        col.startswith('session2') or \
                        col.startswith('session3') or \
                        col.startswith('session4') or \
                        col.startswith('session5') or \
                        col.startswith('session6') or \
                        col.startswith('session7') or \
                        col.startswith('session8') or \
                        col.startswith('session9') or \
                        col.startswith('session10') or \
                        col.startswith('session11') or \
                        col.startswith('elapsed_time0') or \
                        col.startswith('elapsed_time1') or \
                        col.startswith('elapsed_time2') or \
                        col.startswith('elapsed_time3') or \
                        col.startswith('elapsed_time4') or \
                        col.startswith('elapsed_time5') or \
                        col.startswith('elapsed_time6') or \
                        col.startswith('elapsed_time7') or \
                        col.startswith('elapsed_time8') or \
                        col.startswith('elapsed_time9') or \
                        col.startswith('elapsed_time10') or \
                        col.startswith('elapsed_time11')):
