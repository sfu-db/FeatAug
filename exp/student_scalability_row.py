...
def load_data(question_number, log_all, train_all, SAMPLE_ROW_NUM: int = 10000):
    ...
    random.seed(42)
    sampled_session = random.sample(log_question['session'].unique().tolist(), SAMPLE_ROW_NUM)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', '--sampled_row_num', type=int, default=10000, help = "ML model",required = False)
    ...
    for question_number in [question_num]:
        train_data, train_labels, valid_data, valid_labels, test_data, test_labels, user_log = load_data(question_number, log_all, train_all, SAMPLE_ROW_NUM = sampled_row_num)
